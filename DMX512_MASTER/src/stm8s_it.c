...
extern DMX512_TxState DMX512_TX_nextState;
extern uint8_t DMX512_TX_countAddr;
...
void TIM4_IQRHandler(void)
{
	...
	switch(DMX512_TX_nextState)
	{
		case TxBreak:
			DMX512_TX_nextState = TxStart;
			DMX512_TX_sendBreak();
			break;
			
		case TxStart:
			DMX512_TX_nextState = TxData;
			DMX512_TX_enableTx();
			DMX512_TX_countAddr = 0;
			DMX512_TX_sendData(STARTCODE);
			break;
			
		case TxData:
			if( ++DMX512_TX_countAddr >= MAXCHANNEL)
				DMX512_TX_nextState = TxBreak;
			// else DMX512_NextState = TxData
			DMX512_TX_sendData(packet[DMX512_TX_countAddr-1]);
			break;
	}
